...
-- 「Arubaの人口は103000人です」
SELECT CONCAT(name,'の人口は',population,'です') FROM countries;
...
SELECT SUM(surface_area) AS 'アジアの合計表面積' FROM countries WHERE continent IN ('Asia');
...
SELECT countries.name AS '国名',cities.name AS '地区町村',language
FROM countries 
...
SELECT celebrities.name,countries.name AS '国名'
...
FROM celebrities
WHERE birth 
BETWEEN '1980-1-1' AND '1980-12-31'
UNION
